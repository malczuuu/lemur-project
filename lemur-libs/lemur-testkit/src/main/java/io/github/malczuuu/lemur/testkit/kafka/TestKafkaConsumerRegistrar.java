...
class TestKafkaConsumerRegistrar {
  ...
  private TestKafkaConsumer createClient(String bootstrapServers, String resolvedTopic) {
    ...
    props.put(ConsumerConfig.GROUP_ID_CONFIG, getUniqueGroupId(resolvedTopic));
    props.put(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earliest");
    props.put(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, false);
    KafkaConsumer<String, String> consumer =
        new KafkaConsumer<>(props, new StringDeserializer(), new StringDeserializer());
    return new DefaultTestKafkaConsumer(consumer, resolvedTopic);
  }

  private String getUniqueGroupId(String topic) {
    return "test-consumer." + topic + "." + UUID.randomUUID();
  }
}
